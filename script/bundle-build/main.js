...
// files under internal/build/src/<core>/, then emits two artifacts per core:
//
//   internal/<out>.js       readable, banner-headed, concatenated (safe target)
//   internal/<out>.min.js   fully minified, comment-free (smallest)
//
...
//
// Usage: node main.js   (from script/bundle-build; or `make bundle_build`)
...
const banner = (core) =>
  [
    ...
    `// Edit the sources in internal/build/src/${core}/ and run \`make bundle_build\`.`,
  ].join("\n");
...
const main = async () => {
  for (const { core, out } of targets) {
    ...
    // parse as a module so top-level await is accepted; keep the banner.
    const minified = await minify(body, {
      module: true,
      compress: true,
      mangle: true,
      format: { comments: false, preamble: banner(core) },
    });
    if (minified.error) throw minified.error;
    const minPath = path.join(outDir, `${out}.min.js`);
    fs.writeFileSync(minPath, minified.code + "\n");

    console.log(
      `generated internal/${out}.js (+ .min.js) from ${files} parts, ${features} features, ${steps} steps`,
    );
  }
};
